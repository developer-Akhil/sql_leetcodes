...
# Solution
```
select c.name as country from Person p join Country c on left(p.phone_number,3) = c.country_code
join Calls cl on p.id in (cl.caller_id,cl.callee_id) group by c.name
